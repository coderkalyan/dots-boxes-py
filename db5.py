...
Also, tries to play swastika pattern to make sure large
"chains" do not form, as this can give the user an advantage.
If no completions exist, chooses move at random.
Notes:
...
BOX_USER = myfont.render('U', False, BLUE)
BOX_COMPUTER = myfont.render('C', False, RED)
swastika1 = [(2,6),(10,11),(9,13),(4,5)]
swastika2 = [(1,5),(6,7),(10,14),(8,9)]
...
def disp_board():
    ...
    score_user = score_font.render(f"USER: {score[0]}", False, BLUE)
    w, h = score_font.size(f"USER: {score[0]}")
    SURF.blit(score_user, (size // 2 - w - 10, 10))
    score_comp = score_font.render(f"AI: {score[1]}", False, RED)
    w2, h2 = score_font.size(f"AI: {score[1]}")
...
def get_best_move(possible):
    ...
    # now, we want to prioritize any swastika moves
    # print(swastika1)
    # print(possible)
    for p_move in possible:
        a, b = p_move
        if (a, b) in swastika1 or (b, a) in swastika1:
            return p_move
    for p_move in possible:
        a, b = p_move
        if (a, b) in swastika2 or (b, a) in swastika2:
            return p_move
