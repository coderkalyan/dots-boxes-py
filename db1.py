...
Version 1:
Computer picks random but valid move.
...
moves_done = []
boxes = [Box(i, i+1, i+BOARDSIZE, i+BOARDSIZE+1,OWNER_NONE) for i in range(4)]
boxes.extend([Box(i, i+1, i+BOARDSIZE, i+BOARDSIZE+1, OWNER_NONE) for i in range(8,12)])
...
def check_move_made_box(is_user, id1, id2):
    ...
    for i, box in enumerate(boxes):
        if id1 not in box or id2 not in box:
            continue
        temp = list(box[:])
        temp.remove(id1)
        temp.remove(id2)
